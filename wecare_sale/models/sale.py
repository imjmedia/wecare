...
class SaleOrderLine(models.Model):
    ...
    x_units_per_uom = fields.Float(string="Unidades por Caja", compute="_compute_unidad_por_caja", store=True)
    x_price_per_unit = fields.Float(string="Precio por Pieza", compute="_compute_precio_por_pieza", store=True)

    @api.depends('product_uom')
    def _compute_unidad_por_caja(self):
        for record in self:
