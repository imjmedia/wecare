...
class ReporteComision(models.Model):
    ...
    date = fields.Datetime('Fecha Factura', readonly=True)
    ...
    def init(self):
        ...
        CREATE or REPLACE VIEW comision_venta AS (
         ...
         FROM( SELECT am.id as invoice, am.invoice_date as date, am.partner_id as partner_id,
            am.invoice_user_id as user_id, am.amount_untaxed as subtotal,
            CASE WHEN pp.type = 'vip' AND (am.invoice_date - fecha_factura) <= 30 THEN (am.amount_untaxed*0.015)
            WHEN pp.type = 'vip' AND (am.invoice_date - fecha_factura) <= 120 THEN (am.amount_untaxed*0.0125)
            WHEN pp.type = 'vip' AND (am.invoice_date - fecha_factura) > 120 THEN (am.amount_untaxed*0.00875)
            WHEN pp.type = 'final' AND (am.invoice_date - fecha_factura) <= 30 THEN (am.amount_untaxed*0.03)
            WHEN pp.type = 'final' AND (am.invoice_date - fecha_factura) <= 120 THEN (am.amount_untaxed*0.025)
            WHEN pp.type = 'final' AND (am.invoice_date - fecha_factura) > 120 THEN (am.amount_untaxed*0.0175)
            WHEN pp.type = 'mayoreo' AND (am.invoice_date - fecha_factura) <= 30 THEN (am.amount_untaxed*0.042)
            WHEN pp.type = 'mayoreo' AND (am.invoice_date - fecha_factura) <= 120 THEN (am.amount_untaxed*0.035)
            WHEN pp.type = 'mayoreo' AND (am.invoice_date - fecha_factura) > 120 THEN (am.amount_untaxed*0.0245)
            ...
            where ip.name = 'property_product_pricelist' and am.move_type = 'out_invoice' and am.amount_residual = 0 
            ) as line
            WHERE
            line.state = 'posted'
         )""")